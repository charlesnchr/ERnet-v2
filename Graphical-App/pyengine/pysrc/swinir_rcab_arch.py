    ...
    def __init__(self,
        ...
        if self.upsampler == 'pixelshuffle':
            ...
            self.conv_before_upsample = nn.Sequential(
                nn.Conv2d(embed_dim, num_feat, 3, 1, 1), nn.LeakyReLU(inplace=True))
            self.upsample = Upsample(upscale, num_feat)
            self.conv_last = nn.Conv2d(num_feat, round(pixelshuffleFactor**2*num_feat), 3, 1, 1)
            self.conv_combine = nn.Conv2d(num_in_ch, num_out_ch, 3, 1, 1)

        self.segmentation_decode = nn.Conv2d(num_feat, 4, 1)
        self.task = opt.task
        self.vis = vis
    ...
    def forward(self, x):
        ...
        if self.upsampler == 'pixelshuffle':
            ...
            if self.task == 'segment':
                # torch.save(x,'pre_segmentation_swinir_rcab.pth')
                x = self.segmentation_decode(x) # assumes pixelshuffle = 1
                # torch.save(x,'post_segmentation_swinir_rcab.pth')
            else:
        ...
        # torch.save(x,'post_img_range_swinir_rcab.pth')
